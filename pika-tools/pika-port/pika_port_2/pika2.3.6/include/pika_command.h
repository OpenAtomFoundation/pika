...
class CmdRes {
 public:
  ...
  void AppendStringLen(int ori) { RedisAppendLen(message_, ori, "$"); }
  void AppendArrayLen(int ori) { RedisAppendLen(message_, ori, "*"); }
  void AppendInteger(int ori) { RedisAppendLen(message_, ori, ":"); }
...
class Cmd {
 public:
  ...
  virtual std::string ToBinlog(const PikaCmdArgsType& argv, const std::string& server_id,
 ...
 protected:
  ...
  void AppendAffiliatedInfo(std::string& res, const std::string& server_id, const std::string& binlog_info,
