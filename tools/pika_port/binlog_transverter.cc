...
bool PortBinlogTransverter::PortBinlogDecode(PortBinlogType type, const std::string& binlog, PortBinlogItem* binlog_item) {
  ...
  if (binlog_type != type) {
    LOG(ERROR) << "PortBinlog Item type error, expect type:" << type << " actualy type: " << binlog_type;
    return false;
  }
  ...
  if (binlog_str.size() >= content_length) {
    ...
    LOG(ERROR) << "PortBinlog Item get content error, expect length:" << content_length << " left length:" << binlog_str.size();
    return false;
  }
