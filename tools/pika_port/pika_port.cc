...
PikaPort::PikaPort(std::string& master_ip, int master_port, std::string& passwd)
  ...
  for (size_t i = 0; i < thread_num; i++) { 
    senders_.emplace_back(new RedisSender(int(i), g_conf.forward_ip,
	       g_conf.forward_port, g_conf.forward_passwd));
  }

  // Create thread
  binlog_receiver_thread_ = new BinlogReceiverThread(g_conf.local_ip,
                                  g_conf.local_port + 1000, 1000);
...
bool PikaPort::ShouldAccessConnAsMaster(const std::string& ip) {
  slash::RWLock l(&state_protector_, false);
  LOG(INFO) << "ShouldAccessConnAsMaster, repl_state_: " << PikaState(repl_state_)
