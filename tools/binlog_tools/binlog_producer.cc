...
Status BinlogProducer::EmitPhysicalRecord(RecordType t, const char *ptr, size_t n, int *temp_pro_offset) {
    Status s;
    assert(n <= 0xffffff);
...
Status ReadableBinlogProducer::EmitPhysicalRecord(RecordType t, const char *ptr, size_t n, int *temp_pro_offset, uint64_t produce_time) {
    Status s;
    assert(n <= 0xffffff);
...
Status ReadableBinlogProducer::Produce(const Slice &item, int *temp_pro_offset, uint64_t produce_time) {
  Status s;
  const char *ptr = item.data();
  size_t left = item.size();
  bool begin = true;
  ...
  *temp_pro_offset = version_->pro_offset_;
  do {
    ...
    s = EmitPhysicalRecord(type, ptr, fragment_length, temp_pro_offset, produce_time);
    ptr += fragment_length;
    left -= fragment_length;
    begin = false;
  } while (s.ok() && left > 0);
...
Status ReadableBinlogProducer::Put(const std::string& item, uint64_t produce_time) {
  Status s;
  /* Check to roll log file */
  if (queue_ == NULL) {
      return Status::Corruption("no file is loaded yet");
  ...
  int pro_offset;
  s = Produce(Slice(item.data(), item.size()), &pro_offset, produce_time);
  if (s.ok()) {
    slash::RWLock(&(version_->rwlock_), true);
    version_->pro_offset_ = pro_offset;
    version_->StableSave();
