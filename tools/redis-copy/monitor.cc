...
enum Status {
  Begin = 0,
  Wait,    // skip character on src
  Copy,    // upon this status copy from src to des
  Escape,  // read backslash
  Error
};
enum Condition { Quotation = 0, Backslash, Othersit };
...
static bool remote_conn(redisContext **conn, const std::string &hostname, const std::string &port,
...
static bool reply_process(const redisReply *reply, redisContext *des_conn) {
  ...
  redisReply *des_reply =
      reinterpret_cast<redisReply *>(redisCommandArgv(des_conn, argv.size(), &(argv[0]), &(argv_len[0])));
  if (des_reply == NULL) {
...
static void usage() {
  fprintf(stderr,
          "VERSION:%s \n"
          "DESCRIPTION:\n"
          " - Redis monitor copy tool: monitor redis server indicated by src_host, src_port, src_auth and send to des "
          "server\n"
          "Parameters: \n"
          "   -s: source server \n"
