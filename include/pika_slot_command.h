...
class PikaMigrate {
 public:
  ...
  int MigrateKey(const std::string& host, const int port, int timeout,
                 const std::string& key, const char type, std::string& detail,
                 std::shared_ptr<Slot> slot);
  void CleanMigrateClient();

  void Lock() { mutex_.lock(); }
  int Trylock() { return mutex_.try_lock(); }
  void Unlock() { mutex_.unlock(); }
  net::NetCli* GetMigrateClient(const std::string& host, const int port,
                                int timeout);
 ...
 private:
  ...
  int MigrateSend(net::NetCli* migrate_cli, const std::string& key,
                  const char type, std::string& detail,
                  const std::shared_ptr<Slot>& slot);
  bool MigrateRecv(net::NetCli* migrate_cli, int need_receive,
                   std::string& detail);

  int ParseKey(const std::string& key, const char type, std::string& wbuf_str,
               const std::shared_ptr<Slot>& slot);
  int64_t TTLByType(const char key_type, const std::string& key,
                    const std::shared_ptr<Slot>& slot);
...
class SlotsReloadCmd : public Cmd {
 public:
  SlotsReloadCmd(const std::string& name, int arity, uint16_t flag)
      : Cmd(name, arity, flag) {}
  void Do(std::shared_ptr<Slot> slot) override;
  void Split(std::shared_ptr<Slot> slot, const HintKeys& hint_keys) override{};
  void Merge() override{};
...
class SlotsReloadOffCmd : public Cmd {
 public:
  SlotsReloadOffCmd(const std::string& name, int arity, uint16_t flag)
      : Cmd(name, arity, flag) {}
  void Do(std::shared_ptr<Slot> slot) override;
  void Split(std::shared_ptr<Slot> slot, const HintKeys& hint_keys) override{};
  void Merge() override{};
...
class SlotsCleanupCmd : public Cmd {
 public:
  SlotsCleanupCmd(const std::string& name, int arity, uint16_t flag)
      : Cmd(name, arity, flag) {}
  void Do(std::shared_ptr<Slot> slot) override;
  void Split(std::shared_ptr<Slot> slot, const HintKeys& hint_keys) override{};
  void Merge() override{};
...
class SlotsCleanupOffCmd : public Cmd {
 public:
  SlotsCleanupOffCmd(const std::string& name, int arity, uint16_t flag)
      : Cmd(name, arity, flag) {}
  void Do(std::shared_ptr<Slot> slot) override;
  void Split(std::shared_ptr<Slot> slot, const HintKeys& hint_keys) override{};
  void Merge() override{};
