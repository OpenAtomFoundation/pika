...
class PfAddCmd : public Cmd {
 public:
  PfAddCmd(const std::string& name, int32_t arity, uint16_t flag) : Cmd(name, arity, flag) {}
...
class PfCountCmd : public Cmd {
 public:
  PfCountCmd(const std::string& name, int32_t arity, uint16_t flag) : Cmd(name, arity, flag) {}
  void Do(std::shared_ptr<Slot> slot = nullptr) override;
  void Split(std::shared_ptr<Slot> slot, const HintKeys& hint_keys) override {};
  void Merge() override {};
...
class PfMergeCmd : public Cmd {
 public:
  PfMergeCmd(const std::string& name, int32_t arity, uint16_t flag) : Cmd(name, arity, flag) {
