...
class PikaParseSendThread : public net::Thread {
 public:
  PikaParseSendThread(PikaMigrateThread *migrate_thread,
                      const std::shared_ptr<Slot> &slot_);
  ~PikaParseSendThread();

  bool Init(const std::string &ip, int64_t port, int64_t timeout_ms,
            int64_t mgrtkeys_num);
  void ExitThread(void);

 private:
  int MigrateOneKey(net::NetCli *cli, const std::string key,
                    const char key_type, bool async);
  void DelKeysAndWriteBinlog(
...
class PikaMigrateThread : public net::Thread {
 public:
  ...
  bool ReqMigrateBatch(const std::string &ip, int64_t port, int64_t time_out,
                       int64_t slot_num, int64_t keys_num,
                       const std::shared_ptr<Slot> &slot);
  int ReqMigrateOne(const std::string &key, const std::shared_ptr<Slot> &slot);
  void GetMigrateStatus(std::string *ip, int64_t *port, int64_t *slot,
                        bool *migrating, int64_t *moved, int64_t *remained);
 ...
 private:
  ...
  void ReadSlotKeys(const std::string &slotKey, int64_t need_read_num,
