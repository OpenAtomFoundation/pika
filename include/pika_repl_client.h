...
struct ReplClientWriteDBTaskArg {
  ...
  ReplClientWriteDBTaskArg(std::shared_ptr<Cmd> _cmd_ptr, const LogOffset& _offset, std::string _db_name,
                           uint32_t _slot_id)
      : cmd_ptr(std::move(_cmd_ptr)), offset(_offset), db_name(std::move(_db_name)), slot_id(_slot_id) {}
...
class PikaReplClient {
 public:
  ...
  pstd::Status SendSlotTrySync(const std::string& ip, uint32_t port, const std::string& db_name, uint32_t slot_id,
                               const BinlogOffset& boffset, const std::string& local_ip);
  pstd::Status SendSlotBinlogSync(const std::string& ip, uint32_t port, const std::string& db_name, uint32_t slot_id,
