...
struct ReplClientWriteBinlogTaskArg {
  ...
  ReplClientWriteBinlogTaskArg(
      ...
      : res(std::move(_res)),
        conn(std::move(_conn)),
        res_private_data(_res_private_data),
        worker(_worker) {}
};
...
struct ReplClientWriteDBTaskArg {
  ...
  std::string db_name;
  uint32_t slot_id;
  ReplClientWriteDBTaskArg(std::shared_ptr<Cmd> _cmd_ptr,
                           const LogOffset& _offset, std::string _db_name,
                           uint32_t _slot_id)
...
class PikaReplClient {
 public:
  ...
  pstd::Status SendSlotDBSync(const std::string& ip, uint32_t port,
                              const std::string& db_name, uint32_t slot_id,
                              const BinlogOffset& boffset,
                              const std::string& local_ip);
  pstd::Status SendSlotTrySync(const std::string& ip, uint32_t port,
                               const std::string& db_name, uint32_t slot_id,
                               const BinlogOffset& boffset,
                               const std::string& local_ip);
  pstd::Status SendSlotBinlogSync(const std::string& ip, uint32_t port,
                                  const std::string& db_name, uint32_t slot_id,
                                  const LogOffset& ack_start,
                                  const LogOffset& ack_end,
                                  const std::string& local_ip,
                                  bool is_first_send);
  pstd::Status SendRemoveSlaveNode(const std::string& ip, uint32_t port,
                                   const std::string& db_name, uint32_t slot_id,
                                   const std::string& local_ip);
