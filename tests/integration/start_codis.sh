...
CODIS_GROUP_1_SLAVE=127.0.0.1:8001
CODIS_GROUP_2_MASTER=127.0.0.1:8002
CODIS_GROUP_2_SLAVE=127.0.0.1:8003
...
sed -i '' -e 's|databases : 1|databases : 2|' -e 's|port : 9221|port : 8002|' -e 's|log-path : ./log/|log-path : ./codis_data_3/log/|' -e 's|db-path : ./db/|db-path : ./codis_data_3/db/|' -e 's|dump-path : ./dump/|dump-path : ./codis_data_3/dump/|' -e 's|pidfile : ./pika.pid|pidfile : ./codis_data_4/pika.pid|' -e 's|db-sync-path : ./dbsync/|db-sync-path : ./codis_data_3/dbsync/|' -e 's|#daemonize : yes|daemonize : yes|' ./pika_8002.conf
sed -i '' -e 's|databases : 1|databases : 2|' -e 's|port : 9221|port : 8003|' -e 's|log-path : ./log/|log-path : ./codis_data_4/log/|' -e 's|db-path : ./db/|db-path : ./codis_data_4/db/|' -e 's|dump-path : ./dump/|dump-path : ./codis_data_4/dump/|' -e 's|pidfile : ./pika.pid|pidfile : ./codis_data_4/pika.pid|' -e 's|db-sync-path : ./dbsync/|db-sync-path : ./codis_data_4/dbsync/|' -e 's|#daemonize : yes|daemonize : yes|' ./pika_8003.conf
# Start three nodes
../../output/pika -c ./pika_8000.conf
../../output/pika -c ./pika_8001.conf
../../output/pika -c ./pika_8002.conf
../../output/pika -c ./pika_8003.conf
...
./bin/codis-admin --dashboard=$CODIS_DASHBOARD_ADDR --group-add --gid=1 --addr=$CODIS_GROUP_1_SLAVE

./bin/codis-admin --dashboard=$CODIS_DASHBOARD_ADDR --group-add --gid=2 --addr=$CODIS_GROUP_2_MASTER
./bin/codis-admin --dashboard=$CODIS_DASHBOARD_ADDR --group-add --gid=2 --addr=$CODIS_GROUP_2_SLAVE

./bin/codis-admin --dashboard=$CODIS_DASHBOARD_ADDR --sync-action --create --addr=$CODIS_GROUP_1_SLAVE
./bin/codis-admin --dashboard=$CODIS_DASHBOARD_ADDR --sync-action --create --addr=$CODIS_GROUP_2_SLAVE
