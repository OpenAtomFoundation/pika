...
sed -i '' -e 's|requirepass :|requirepass : requirepass|' -e 's|masterauth :|masterauth : requirepass|' -e 's|# userpass :|userpass : userpass|' -e 's|userblacklist :|userblacklist : flushall,flushdb|' -e 's|port : 9221|port : 9261|' -e 's|log-path : ./log/|log-path : ./acl1_data/log/|' -e 's|db-path : ./db/|db-path : ./acl1_data/db/|' -e 's|dump-path : ./dump/|dump-path : ./acl1_data/dump/|' -e 's|pidfile : ./pika.pid|pidfile : ./acl1_data/pika.pid|' -e 's|db-sync-path : ./dbsync/|db-sync-path : ./acl1_data/dbsync/|' -e 's|#daemonize : yes|daemonize : yes|' ./pika_acl_both_password.conf
...
sed -i '' -e 's|requirepass :|requirepass : requirepass|' -e 's|masterauth :|masterauth : requirepass|' -e 's|# userpass :|userpass : userpass|' -e 's|userblacklist :|userblacklist : flushall,flushdb|' -e 's|port : 9221|port : 9281|'  -e 's|log-path : ./log/|log-path : ./acl3_data/log/|' -e 's|db-path : ./db/|db-path : ./acl3_data/db/|' -e 's|dump-path : ./dump/|dump-path : ./acl3_data/dump/|' -e 's|pidfile : ./pika.pid|pidfile : ./acl3_data/pika.pid|' -e 's|db-sync-path : ./dbsync/|db-sync-path : ./acl3_data/dbsync/|' -e 's|#daemonize : yes|daemonize : yes|' ./pika_has_other_acl_user.conf
