...
#    }
    ...
    test {ACLs can include single subcommands} {
        ...
        r ACL setuser newuser +client|setname
...
#    }
    ...
    test {ACL GETUSER is able to translate back command permissions} {
        ...
        r ACL setuser newuser reset +@all ~* -@string +incr
        set cmdstr [dict get [r ACL getuser newuser] commands]
        ...
        r ACL setuser newuser reset +@string -incr +acl
        set cmdstr [dict get [r ACL getuser newuser] commands]
        assert_match {*-@all*} $cmdstr
        assert_match {*+@string*} $cmdstr
        assert_match {*-incr*} $cmdstr
...
#    } {*NOAUTH*}
    ...
    test {When an authentication chain is used in the HELLO cmd, the last auth cmd has precedence} {
        ...
        r HELLO 2 AUTH secure-user supass AUTH secure-user2 supass AUTH secure-user1 supass
        assert_equal [r ACL whoami] {secure-user1}
        ...
        assert_equal [r ACL whoami] {secure-user2}
    }
    ...
    test {When a setname chain is used in the HELLO cmd, the last setname cmd has precedence} {
        ...
        assert_equal [r client getname] {client4}
        ...
        assert_equal [r client getname] {client6}
    }
    ...
    test {ACL load non-existing configured ACL file} {
       ...
       set err
    } {*not configured to use an ACL file*}
...
set base_path ${server_path}/
exec cp -f tests/assets/user.acl $base_path
set acl_file ${base_path}user.acl
start_server [list overrides [list "dir" $server_path "acl-pubsub-default" "allchannels" "aclfile" $acl_file ] tags [list "external:skip"]] {
...
set base_path ${server_path}/
exec cp -f tests/assets/nodefaultuser.acl $base_path
exec cp -f tests/assets/default.conf $server_path
set acl_file ${base_path}nodefaultuser.acl
start_server [list overrides [list "dir" $server_path "aclfile" $acl_file] tags [list "external:skip"]] {
...
exec cp -f tests/assets/default.conf $server_path
set acl_file ${base_path}nodefaultuser.acl
start_server [list overrides [list "dir" $server_path "acl-pubsub-default" "resetchannels" "aclfile" $acl_file] tags [list "external:skip"]] {
...
start_server [list overrides [list "dir" $server_path "aclfile" $acl_file] tags [list "external:skip"]] {
    ...
    test {Test loading an ACL file with duplicate users} {
        ...
        exec echo $corruption >> ${base_path}user.acl
        catch {r ACL LOAD} err
...
#Because the tcl test was slow and there was a problem with restarting the service, everything was removed
