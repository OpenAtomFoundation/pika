start_server {
    ...
    test {EVAL - Is Lua affecting the currently selected DB?} {
        r select 5
        r set mykey "this is DB 5"
        r select 6
        r set mykey "this is DB 6"
        r eval {return redis.pcall('get','mykey')} 0
    } {this is DB 6}

    test {EVAL - Is Lua seleced DB retained?} {
        r eval {return redis.pcall('select','5')} 0
        r get mykey
    } {this is DB 5}

    # TODO unsupport configure now
    # if 0 {
    ...
    test {EVAL - Scripts can't run certain commands} {
        set e {}
            ...
            r eval "redis.pcall('time'); return redis.pcall('set','x','ciao')" 0
        } e
        set e
    } {*not allowed after*}
