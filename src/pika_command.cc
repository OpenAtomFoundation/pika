...
void InitCmdTable(CmdTable* cmd_table) {
  ...
  std::unique_ptr<Cmd> blpopptr = std::make_unique<BLPopCmd>(kCmdNameBLPop, -3, kCmdFlagsMayDfferWriteBinlog | kCmdFlagsSingleSlot | kCmdFlagsList);
  ...
  std::unique_ptr<Cmd> brpopptr = std::make_unique<BRPopCmd>(kCmdNameBRpop, -3, kCmdFlagsMayDfferWriteBinlog | kCmdFlagsSingleSlot | kCmdFlagsList);
...
void Cmd::InternalProcessCommand(const std::shared_ptr<Slot>& slot,
  ...
  if (is_write() || is_may_defer_write_binlog()) {
...
void Cmd::DoBinlog(const std::shared_ptr<SyncMasterSlot>& slot) {
  if (res().ok() && (is_write() || is_may_defer_write_binlog()) && g_pika_conf->write_binlog()) {
...
bool Cmd::is_may_defer_write_binlog() const { return ((flag_ & kCmdFlagsMayDfferWriteBinlog) == kCmdFlagsMayDfferWriteBinlog); }
