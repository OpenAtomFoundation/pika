...
PikaReplClientThread::PikaReplClientThread(int cron_interval,
                                           int keepalive_timeout)
    : ClientThread(&conn_factory_, cron_interval, keepalive_timeout, &handle_,
                   nullptr) {}

void PikaReplClientThread::ReplClientHandle::FdClosedHandle(
  ...
  if (ip == g_pika_server->master_ip() &&
      ...
      PIKA_REPL_ERROR !=
          g_pika_server
              ->repl_state()) {  // if state machine in error state, no retry
...
void PikaReplClientThread::ReplClientHandle::FdTimeoutHandle(
    int fd, const std::string& ip_port) const {
  ...
  if (ip == g_pika_server->master_ip() &&
      ...
      PikaReplicaManager::CheckSlaveSlotState(
          ip, port)) {  // if state machine in error state, no retry
