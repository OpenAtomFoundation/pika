...
void SlaveofCmd::DoInitial() {
  ...
  if (argv_.size() == 3 && (strcasecmp(argv_[1].data(), "no") == 0) &&
      (strcasecmp(argv_[2].data(), "one") == 0)) {
    is_none_ = true;
    return;
  }
...
void SlaveofCmd::Do(std::shared_ptr<Slot> slot) {
  ...
  if (sm_ret) {
    ...
    res_.SetRes(CmdRes::kErrOther,
                "Server is not in correct state for slaveof");
  }
}
...
void DbSlaveofCmd::DoInitial() {
  ...
  if (argv_.size() == 4) {
    if ((strcasecmp(argv_[2].data(), "no") == 0) &&
        (strcasecmp(argv_[3].data(), "one") == 0)) {
      is_none_ = true;
      return;
    }

    if ((pstd::string2int(argv_[2].data(), argv_[2].size(), &filenum_) == 0) ||
        filenum_ < 0) {
      res_.SetRes(CmdRes::kInvalidInt);
      return;
    }
    if ((pstd::string2int(argv_[3].data(), argv_[3].size(), &offset_) == 0) ||
        offset_ < 0) {
      res_.SetRes(CmdRes::kInvalidInt);
      return;
    }
...
void DbSlaveofCmd::Do(std::shared_ptr<Slot> slot) {
  ...
  if (is_none_) {
    ...
    if (slave_slot->State() == ReplState::kNoConnect ||
        slave_slot->State() == ReplState::kError ||
      ...
      ReplState state =
          force_sync_ ? ReplState::kTryDBSync : ReplState::kTryConnect;
...
void CompactCmd::DoInitial() {
  ...
  if (g_pika_server->IsKeyScaning()) {
    res_.SetRes(CmdRes::kErrOther,
                "The info keyspace operation is executing, Try again later");
    return;
  }
...
void CompactCmd::Do(std::shared_ptr<Slot> slot) {
  if (strcasecmp(struct_type_.data(), "all") == 0) {
    ...
    g_pika_server->DoSameThingSpecificDB(TaskType::kCompactStrings,
                                         compact_dbs_);
  } else if (strcasecmp(struct_type_.data(), "hash") == 0) {
    g_pika_server->DoSameThingSpecificDB(TaskType::kCompactHashes,
                                         compact_dbs_);
...
void PurgelogstoCmd::DoInitial() {
  ...
  int64_t num = 0;
  if ((pstd::string2int(str_num.data(), str_num.size(), &num) == 0) ||
      num < 0) {
    res_.SetRes(CmdRes::kInvalidParameter);
    return;
  }
...
void SelectCmd::DoInitial() {
  ...
  if (index < 0 || index >= g_pika_conf->databases()) {
    res_.SetRes(CmdRes::kInvalidIndex,
                kCmdNameSelect + " DB index is out of range");
    return;
  }
  db_name_ = "db" + argv_[1];
  if (!g_pika_server->IsDBExist(db_name_)) {
...
std::string FlushallCmd::ToBinlog(uint32_t exec_time, uint32_t term_id,
...
void ClientCmd::DoInitial() {
  ...
  if ((strcasecmp(argv_[1].data(), "list") == 0) && argv_.size() == 2) {
    ...
    if ((strcasecmp(argv_[2].data(), "order") == 0) &&
        (strcasecmp(argv_[3].data(), "by") == 0)) {
      info_ = argv_[4];
    } else {
      res_.SetRes(CmdRes::kErrOther,
  ...
  if (strcasecmp(operation_.data(), "list") == 0) {
    ...
    while (iter != clients.end()) {
      snprintf(buf, sizeof(buf), "addr=%s fd=%d idle=%lld\n",
               ...
               iter->last_interaction == 0
                   ? 0
                   : now.tv_sec - iter->last_interaction);  // NOLINT
      reply.append(buf);
      iter++;
    }
    res_.AppendString(reply);
  } else if ((strcasecmp(operation_.data(), "kill") == 0) &&
             (strcasecmp(info_.data(), "all") == 0)) {
  ...
  if (is_local()) {
    ...
    if (conn) {
      if (conn->ip_port().find("127.0.0.1") == std::string::npos &&
        ...
        res_.SetRes(CmdRes::kErrOther,
                    kCmdNameShutdown + " should be localhost");
      }
    } else {
      LOG(WARNING) << name_ << " weak ptr is empty";
  ...
  if (strcasecmp(argv_[1].data(), kAllSection.data()) == 0) {
    ...
    if (argv_[2] == "1") {
      if (g_pika_server->IsCompacting()) {
        res_.SetRes(CmdRes::kErrOther,
                    "The compact operation is executing, Try again later");
      } else {
        rescan_ = true;
      }
    } else if (argv_[2] == "off") {
...
void InfoCmd::InfoServer(std::string& info) {
  ...
  tmp_stream << "pika_build_compile_date: " << pika_build_compile_date
             << "\r\n";
  tmp_stream << "os:" << host_info.sysname << " " << host_info.release << " "
             << host_info.machine << "\r\n";
  tmp_stream << "arch_bits:"
             << (reinterpret_cast<char*>(&host_info.machine) +
                 strlen(host_info.machine) - 2)
             << "\r\n";
  ...
  tmp_stream << "uptime_in_seconds:"
             << (current_time_s - g_pika_server->start_time_s()) << "\r\n";
  tmp_stream << "uptime_in_days:"
             << (current_time_s / (24 * 3600) -
                 g_pika_server->start_time_s() / (24 * 3600) + 1)
             << "\r\n";
...
void InfoCmd::InfoExecCount(std::string& info) {
  ...
  std::unordered_map<std::string, uint64_t> command_exec_count_db =
      g_pika_server->ServerExecCountDB();
...
void InfoCmd::InfoShardingReplication(std::string& info) {
  ...
  uint32_t slave_num =
      g_pika_server->GetShardingSlaveListString(slave_list_string);
  if (slave_num != 0U) {
...
void InfoCmd::InfoReplication(std::string& info) {
  ...
  for (const auto& db_item : g_pika_server->dbs_) {
    ...
    for (const auto& slot_item : db_item.second->slots_) {
      std::shared_ptr<SyncSlaveSlot> slave_slot =
          g_pika_rm->GetSyncSlaveSlotByName(SlotInfo(
  ...
  switch (host_role) {
    ...
    case PIKA_ROLE_SINGLE:
    case PIKA_ROLE_MASTER:
      tmp_stream << "connected_slaves:"
                 << g_pika_server->GetSlaveListString(slaves_list_str) << "\r\n"
                 << slaves_list_str;
  }
  ...
  for (const auto& t_item : g_pika_server->dbs_) {
    ...
    for (const auto& p_item : t_item.second->slots_) {
      ...
      if (!master_slot) {
        LOG(WARNING) << "Sync Master Slot: " << db_name << ":" << slot_id
                     << ", NotFound";
        continue;
      }
...
void InfoCmd::InfoKeyspace(std::string& info) {
  if (off_) {
    g_pika_server->DoSameThingSpecificDB(TaskType::kStopKeyScan,
                                         keyspace_scan_dbs_);
    info.append("OK\r\n");
    return;
  }
  ...
  for (const auto& db_item : g_pika_server->dbs_) {
    if (keyspace_scan_dbs_.empty() ||
        keyspace_scan_dbs_.find(db_item.first) != keyspace_scan_dbs_.end()) {
      ...
      tmp_stream << db_name << " Strings_keys=" << key_infos[0].keys
                 << ", expires=" << key_infos[0].expires
                 << ", invalid_keys=" << key_infos[0].invaild_keys << "\r\n";
      tmp_stream << db_name << " Hashes_keys=" << key_infos[1].keys
                 << ", expires=" << key_infos[1].expires
                 << ", invalid_keys=" << key_infos[1].invaild_keys << "\r\n";
      tmp_stream << db_name << " Lists_keys=" << key_infos[2].keys
                 << ", expires=" << key_infos[2].expires
                 << ", invalid_keys=" << key_infos[2].invaild_keys << "\r\n";
      tmp_stream << db_name << " Zsets_keys=" << key_infos[3].keys
                 << ", expires=" << key_infos[3].expires
                 << ", invalid_keys=" << key_infos[3].invaild_keys << "\r\n";
      tmp_stream << db_name << " Sets_keys=" << key_infos[4].keys
                 << ", expires=" << key_infos[4].expires
  ...
  if (rescan_) {
    g_pika_server->DoSameThingSpecificDB(TaskType::kStartKeyScan,
                                         keyspace_scan_dbs_);
  }
}
...
void InfoCmd::InfoData(std::string& info) {
  ...
  for (const auto& db_item : g_pika_server->dbs_) {
    ...
    for (const auto& slot_item : db_item.second->slots_) {
      ...
      slot_item.second->db()->GetUsage(
          storage::PROPERTY_TYPE_ROCKSDB_CUR_SIZE_ALL_MEM_TABLES,
          &memtable_usage);
      slot_item.second->db()->GetUsage(
          storage::PROPERTY_TYPE_ROCKSDB_ESTIMATE_TABLE_READER_MEM,
          &table_reader_usage);
      slot_item.second->db()->GetUsage(
          storage::PROPERTY_TYPE_ROCKSDB_BACKGROUND_ERRORS, &background_errors);
      ...
      for (const auto& item : background_errors) {
        if (item.second != 0) {
          ...
          db_fatal_msg_stream << slot_item.second->GetSlotName() << "/"
                              << item.first;
  ...
  tmp_stream << "used_memory:"
             << (total_memtable_usage + total_table_reader_usage) << "\r\n";
  tmp_stream << "used_memory_human:"
             << ((total_memtable_usage + total_table_reader_usage) >> 20)
             << "M\r\n";
  ...
  tmp_stream << "db_fatal:" << (total_background_errors != 0 ? "1" : "0")
             << "\r\n";
  tmp_stream << "db_fatal_msg:"
             << (total_background_errors != 0 ? db_fatal_msg_stream.str()
                                              : "nullptr")
             << "\r\n";
...
void ConfigCmd::DoInitial() {
  ...
  if (strcasecmp(argv_[1].data(), "get") == 0) {
    if (argc != 3) {
      res_.SetRes(CmdRes::kErrOther,
                  "Wrong number of arguments for CONFIG get");
      return;
    }
    ...
    if (argc != 2) {
      res_.SetRes(CmdRes::kErrOther,
                  "Wrong number of arguments for CONFIG rewrite");
      return;
    }
  } else if (strcasecmp(argv_[1].data(), "resetstat") == 0) {
    if (argc != 2) {
      res_.SetRes(CmdRes::kErrOther,
                  "Wrong number of arguments for CONFIG resetstat");
      return;
    }
  } else {
    res_.SetRes(
...
void ConfigCmd::ConfigGet(std::string& ret) {
  ...
  if (pstd::stringmatch(pattern.data(), "cache-index-and-filter-blocks", 1) !=
    ...
    EncodeString(&config_body,
                 g_pika_conf->cache_index_and_filter_blocks() ? "yes" : "no");
  }
  ...
  if (pstd::stringmatch(pattern.data(), "optimize-filters-for-hits", 1) != 0) {
    ...
    EncodeString(&config_body,
                 g_pika_conf->optimize_filters_for_hits() ? "yes" : "no");
  }

  if (pstd::stringmatch(pattern.data(), "level-compaction-dynamic-level-bytes",
                        1) != 0) {
    elements += 2;
    EncodeString(&config_body, "level-compaction-dynamic-level-bytes");
    EncodeString(
        &config_body,
        g_pika_conf->level_compaction_dynamic_level_bytes() ? "yes" : "no");
  }
  ...
  if (pstd::stringmatch(pattern.data(), "slowlog-write-errorlog", 1) != 0) {
    ...
    EncodeString(&config_body,
                 g_pika_conf->slowlog_write_errorlog() ? "yes" : "no");
  }
  ...
  if (pstd::stringmatch(pattern.data(), "rate-limiter-auto-tuned", 1) != 0) {
    ...
    EncodeString(&config_body,
                 g_pika_conf->rate_limiter_auto_tuned() ? "yes" : "no");
  }
...
void ConfigCmd::ConfigSet(std::string& ret) {
  ...
  if (set_item == "timeout") {
    ...
    if (value == "yes") {
      ...
      ret = "-ERR Invalid argument \'" + value +
            "\' for CONFIG SET 'slowlog-write-errorlog'\r\n";
      return;
    }
    ...
    if (ival <= 0 || ival > kBinlogReadWinMaxSize) {
      ret = "-ERR Argument exceed range \'" + value +
            "\' for CONFIG SET 'sync-window-size'\r\n";
      return;
    }
...
void MonitorCmd::Do(std::shared_ptr<Slot> slot) {
  ...
  g_pika_server->AddMonitorClient(
...
void DbsizeCmd::Do(std::shared_ptr<Slot> slot) {
  ...
  if (!db) {
    ...
    int64_t dbsize = key_infos[0].keys + key_infos[1].keys + key_infos[2].keys +
                     key_infos[3].keys + key_infos[4].keys;
...
void DelbackupCmd::Do(std::shared_ptr<Slot> slot) {
  ...
  for (auto& i : dump_dir) {
    ...
    if (g_pika_server->CountSyncSlaves() == 0) {
      ...
      LOG(INFO) << "Syncing, can not delete " << dump_dir_name << " dump file"
                << std::endl;
    }
  }
  res_.SetRes(CmdRes::kOk);
...
void SlowlogCmd::DoInitial() {
  ...
  } else if ((argv_.size() == 2 || argv_.size() == 3) &&
             (strcasecmp(argv_[1].data(), "get") == 0)) {
    condition_ = SlowlogCmd::kGET;
    if (argv_.size() == 3 &&
        (pstd::string2int(argv_[2].data(), argv_[2].size(), &number_) == 0)) {
      res_.SetRes(CmdRes::kInvalidInt);
      return;
    }
  } else {
...
void HelloCmd::Do(std::shared_ptr<Slot> slot) {
  ...
  switch (host_role) {
    ...
    case PIKA_ROLE_MASTER | PIKA_ROLE_SLAVE:
      ...
      break;
    default:
      LOG(INFO) << "unknown role" << host_role << " client ip:port "
                << conn->ip_port();
      return;
  }
...
bool CommandCmd::CommandFieldCompare::operator()(const std::string& a,
...
CommandCmd::EncodablePtr CommandCmd::EncodableInt::MergeFrom(
    const CommandCmd::EncodablePtr& other) const {
...
CommandCmd::EncodablePtr CommandCmd::EncodableString::MergeFrom(
    const CommandCmd::EncodablePtr& other) const {
...
CmdRes& CommandCmd::EncodableMap::EncodeTo(CmdRes& res, const Map& map,
  ...
  for (const auto& kv : map) {
    ...
    if (auto iter = specialization.find(kv.first);
        iter != specialization.end()) {
...
CmdRes& CommandCmd::EncodableMap::EncodeTo(CmdRes& res) const {
  return EncodeTo(res, values_);
}

CommandCmd::EncodablePtr CommandCmd::EncodableMap::MergeFrom(
    const CommandCmd::EncodablePtr& other) const {
...
CommandCmd::EncodablePtr CommandCmd::EncodableArray::MergeFrom(
    const CommandCmd::EncodablePtr& other) const {
...
CommandCmd::EncodablePtr CommandCmd::EncodableStatus::MergeFrom(
    const CommandCmd::EncodablePtr& other) const {
...
const std::unordered_map<std::string, int>
    CommandCmd::CommandFieldCompare::kFieldNameOrder{
...
void CommandCmd::Do(std::shared_ptr<Slot> slots) {
  ...
  if (cmds_begin_ == cmds_end_) {
    cmds = kCommandDocs;
    specializations.insert(kPikaSpecialization.cbegin(),
                           kPikaSpecialization.cend());
  } else {
    for (auto iter = cmds_begin_; iter != cmds_end_; ++iter) {
      if (auto cmd = kCommandDocs.find(*iter); cmd != kCommandDocs.end()) {
        cmds.insert(*cmd);
      }
      if (auto specialization = kPikaSpecialization.find(*iter);
          specialization != kPikaSpecialization.end()) {
  ...
  for (const auto& cmd : cmds) {
    ...
    } else if (auto iter = specializations.find(cmd.first);
               iter == specializations.end()) {
