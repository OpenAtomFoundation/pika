...
static nemo::Status RestoreKV(const std::string &key, const restore_value &dbvalue) {
    if(dbvalue.type != nemo::kKV_DB){
        return nemo::Status::Corruption("dbvalue format error");
    }
...
static nemo::Status RestoreList(const std::string &key, const restore_value &dbvalue) {
    if(dbvalue.type != nemo::kLIST_DB){
        return nemo::Status::Corruption("dbvalue format error");
    }
    ...
    for (; iter != dbvalue.listv.end(); iter++) {
        s = g_pika_server->db()->RPush(key, *iter, &llen);
        if (!s.ok()) {
            return s;
        }
    }
...
static nemo::Status RestoreSet(const std::string &key, const restore_value &dbvalue) {
    if(dbvalue.type != nemo::kSET_DB){
        return nemo::Status::Corruption("dbvalue format error");
    }
    ...
    for (; iter != dbvalue.setv.end(); iter++) {
        s = g_pika_server->db()->SAdd(key, *iter, &res);
        if (!s.ok()) {
            return s;
        }
    }
...
static nemo::Status RestoreZset(const std::string &key, const restore_value &dbvalue) {
    if(dbvalue.type != nemo::kZSET_DB){
        return nemo::Status::Corruption("dbvalue format error");
    }
    ...
    std::vector<nemo::SM>::const_iterator iter = dbvalue.zsetv.begin();
    for (; iter != dbvalue.zsetv.end(); iter++) {
        s = g_pika_server->db()->ZAdd(key, iter->score, iter->member, &res);;
        if (!s.ok()) {
            return s;
        }
    }
...
static nemo::Status RestoreHash(const std::string &key, const restore_value &dbvalue) {
    if(dbvalue.type != nemo::kHASH_DB){
        return nemo::Status::Corruption("dbvalue format error");
    }

    nemo::Status s;

    std::vector<nemo::FV>::const_iterator iter = dbvalue.hashv.begin();
    for (; iter != dbvalue.hashv.end(); iter++) {
        s = g_pika_server->db()->HSet(key, iter->field, iter->val);
        if (!s.ok()) {
            return s;
        }
    }
...
slash::Status SlotsrestoreCmd::BinlogPut(pink::RedisCmdArgsType &argv){
        ...
        std::string binlog = ToBinlog(
                argv,
                g_pika_conf->server_id(),
                binlog_info,
                false);
        slash::Status s = g_pika_server->logger_->Put(binlog);
...
void SlotsrestoreCmd::WriteCommandToBinlog(const std::string &key, const int64_t ttlms, const restore_value &dbvalue) {
    pink::RedisCmdArgsType argv;
    ...
    if (argv.size() > 0) {
            ...
            if (ttlms > 0){
                    ...
                    argv.push_back(key);
                    char buf[100];
                    int64_t expireat = time(nullptr) + ttlms / 1000;
...
void SlotsrestoreCmd::Do() {
    ...
    for (; iter != restore_keys_.end(); iter++) {
            ...
            case nemo::kLIST_DB :
                {
                    ...
                    int64_t count = 0;
                    s = g_pika_server->db()->Del(iter->key, &count);
                    if (!s.ok()) {
                        res_.SetRes(CmdRes::kErrOther, s.ToString());
                        return;
                    }
        ...
        if (iter->ttlms > 0 && s.ok()){
            int64_t res = 0;
            s = g_pika_server->db()->Expire(iter->key, iter->ttlms/1000, &res);
        }
