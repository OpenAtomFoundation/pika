...
void PfAddCmd::Do(std::shared_ptr<DB> db) {
  bool update = false;
  rocksdb::Status s = db->storage()->PfAdd(key_, values_, &update);
  if (s.ok() && update) {
...
void PfCountCmd::Do(std::shared_ptr<DB> db) {
  int64_t value_ = 0;
  rocksdb::Status s = db->storage()->PfCount(keys_, &value_);
  if (s.ok()) {
...
void PfMergeCmd::Do(std::shared_ptr<DB> db) {
  rocksdb::Status s = db->storage()->PfMerge(keys_, value_to_dest_);
  if (s.ok()) {
...
void PfMergeCmd::DoBinlog(const std::shared_ptr<SyncMasterDB>& db) {
