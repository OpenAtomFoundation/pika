...
void PikaReplServer::ScheduleBinlogSyncTask(std::string table_partition, const std::shared_ptr<InnerMessage::InnerRequest> req, std::shared_ptr<pink::PbConn> conn, void* req_private_data) {
  size_t index = GetHashIndex(table_partition);
...
void PikaReplServer::ScheduleDbTask(const std::string &key, PikaCmdArgsType* argv, BinlogItem* binlog_item) {
  size_t index = GetHashIndex(key);
  bg_workers_[index]->ScheduleWriteDb(argv, binlog_item);
}

size_t PikaReplServer::GetHashIndex(std::string key) {
  return str_hash(key) % bg_workers_.size();
}
