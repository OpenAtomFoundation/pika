...
PikaWorkerThread::PikaWorkerThread(pink::ConnFactory* conn_factory, int cron_interval)
    : WorkerThread::WorkerThread(conn_factory, cron_interval),
      thread_querynum_(0),
 ...
 */
  ...
  std::map<int, pink::PinkConn*>::iterator iter = conns_.begin();
...
int64_t PikaWorkerThread::ThreadClientList(std::vector<ClientInfo> *clients) {
  slash::RWLock l(&rwlock_, false);
  if (clients != NULL) {
    std::map<int, pink::PinkConn*>::const_iterator iter = conns_.begin();
    while (iter != conns_.end()) {
      clients->push_back(ClientInfo{iter->first, reinterpret_cast<PikaClientConn*>(iter->second)->ip_port(),
          static_cast<int>((reinterpret_cast<PikaClientConn*>(iter->second)->last_interaction()).tv_sec),
          reinterpret_cast<PikaClientConn*>(iter->second)});
      iter++;
    }
  }
  return conns_.size();
