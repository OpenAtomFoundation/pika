...
PikaSlotBinlogSenderThread::PikaSlotBinlogSenderThread(std::string &ip, int port, slash::SequentialFile *queue, uint32_t filenum, uint64_t con_offset)
...
PikaSlotBinlogSenderThread::~PikaSlotBinlogSenderThread() {
  should_exit_ = true;
...
int PikaSlotBinlogSenderThread::trim() {
  slash::Status s;
...
uint64_t PikaSlotBinlogSenderThread::get_next(bool &is_error) {
  uint64_t offset = 0;
  slash::Status s;
  is_error = false;
...
unsigned int PikaSlotBinlogSenderThread::ReadPhysicalRecord(slash::Slice *result) {
  slash::Status s;
...
Status PikaSlotBinlogSenderThread::Consume(std::string &scratch) {
  Status s;
...
Status PikaSlotBinlogSenderThread::Parse(std::string &scratch) {
  scratch.clear();
  Status s;
  uint32_t pro_num;
  uint64_t pro_offset;

  Binlog* logger = g_pika_server->slot_logger_;
  while (!should_exit_) {
    logger->GetProducerStatus(&pro_num, &pro_offset);
    if (filenum_ == pro_num && con_offset_ == pro_offset) {
      //DLOG(INFO) << "===============BinlogSlotSender Parse no new msg, filenum_" << filenum_ << ", con_offset " << con_offset_;
      //DLOG(INFO) << "===============BinlogSlotSender finiahed!!!!!!===========================";
      usleep(10000);
      continue;
    }

    DLOG(INFO) << "===============BinlogSlotSender start Parse a msg               filenum_" << filenum_ << ", con_offset " << con_offset_;
    s = Consume(scratch);

    DLOG(INFO) << "==============BinlogSlotSender after Parse a msg return " << s.ToString() << " filenum_" << filenum_ << ", con_offset " << con_offset_;
    if (s.IsEndFile()) {
      std::string confile = NewFileName(g_pika_server->slot_logger_->filename, filenum_ + 1);
...
// When we encount
void* PikaSlotBinlogSenderThread::ThreadMain() {
  Status s;
  pink::Status result;
