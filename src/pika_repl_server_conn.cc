...
PikaReplServerConn::PikaReplServerConn(int fd, const std::string& ip_port,
                                       net::Thread* thread,
...
void PikaReplServerConn::HandleMetaSyncRequest(void* arg) {
  std::unique_ptr<ReplServerTaskArg> task_arg(
      static_cast<ReplServerTaskArg*>(arg));
  ...
  std::string masterauth =
      meta_sync_request.has_auth() ? meta_sync_request.auth() : "";
  ...
  if (!g_pika_conf->requirepass().empty() &&
    ...
    LOG(INFO) << "Receive MetaSync, Slave ip: " << node.ip()
              << ", Slave port:" << node.port();
    std::vector<DBStruct> db_structs = g_pika_conf->db_structs();
    bool success = g_pika_server->TryAddSlave(node.ip(), node.port(),
                                              conn->fd(), db_structs);
    ...
    if (!success) {
      ...
      InnerMessage::InnerResponse_MetaSync* meta_sync =
          response.mutable_meta_sync();
      meta_sync->set_classic_mode(g_pika_conf->classic_mode());
      for (const auto& db_struct : db_structs) {
        InnerMessage::InnerResponse_MetaSync_DBInfo* db_info =
            meta_sync->add_dbs_info();
...
void PikaReplServerConn::HandleTrySyncRequest(void* arg) {
  ...
  const InnerMessage::BinlogOffset& slave_boffset =
      try_sync_request.binlog_offset();
  ...
  InnerMessage::InnerResponse::TrySync* try_sync_response =
      response.mutable_try_sync();
  try_sync_response->set_reply_code(
      InnerMessage::InnerResponse::TrySync::kError);
  ...
  if (!slot) {
    ...
    LOG(WARNING) << "DB Name: " << db_name << " Slot ID: " << slot_id
                 << " Not Found, TrySync Error";
    pre_success = false;
  } else {
    slot_name = slot->SlotName();
    LOG(INFO) << "Receive Trysync, Slave ip: " << node.ip()
              << ", Slave port:" << node.port() << ", Slot: " << slot_name
              << ", filenum: " << slave_boffset.filenum()
  ...
  if (pre_success && req->has_consensus_meta()) {
    ...
    if (pre_success) {
      ...
      // need to response to outdated pb, new follower count on this response to
      // update term
      if (meta.term() > slot->ConsensusTerm()) {
        LOG(INFO) << "Update " << slot_name << " term from "
                  << slot->ConsensusTerm() << " to " << meta.term();
        slot->ConsensusUpdateTerm(meta.term());
      }
    }
    if (pre_success) {
      pre_success = TrySyncConsensusOffsetCheck(slot, req->consensus_meta(),
                                                &response, try_sync_response);
    }
  } else if (pre_success) {
...
bool PikaReplServerConn::TrySyncUpdateSlaveNode(
  ...
  if (!slot->CheckSlaveNodeExist(node.ip(), node.port())) {
    ...
    if (!s.ok()) {
      try_sync_response->set_reply_code(
          InnerMessage::InnerResponse::TrySync::kError);
      LOG(WARNING) << "Slot: " << slot_name << " TrySync Failed, "
                   << s.ToString();
      return false;
    }
    const std::string ip_port = pstd::IpPortString(node.ip(), node.port());
    ...
    if (!s.ok()) {
      try_sync_response->set_reply_code(
          InnerMessage::InnerResponse::TrySync::kError);
      LOG(WARNING) << "Slot: " << slot_name << ", Get Session id Failed"
                   << s.ToString();
      return false;
    }
    try_sync_response->set_reply_code(
...
bool PikaReplServerConn::TrySyncConsensusOffsetCheck(
  ...
  if (!s.ok()) {
    if (s.IsNotFound()) {
      ...
      try_sync_response->set_reply_code(
          InnerMessage::InnerResponse::TrySync::kSyncPointBePurged);
      return false;
    } else {
...
bool PikaReplServerConn::TrySyncOffsetCheck(
  ...
  if (!s.ok()) {
    try_sync_response->set_reply_code(
        InnerMessage::InnerResponse::TrySync::kError);
    LOG(WARNING) << "Handle TrySync, Slot: " << slot_name
                 << " Get binlog offset error, TrySync failed";
    return false;
  }
  InnerMessage::BinlogOffset* master_slot_boffset =
      try_sync_response->mutable_binlog_offset();
  ...
  if (boffset.filenum < slave_boffset.filenum() ||
      (boffset.filenum == slave_boffset.filenum() &&
       boffset.offset < slave_boffset.offset())) {
    try_sync_response->set_reply_code(
        InnerMessage::InnerResponse::TrySync::kSyncPointLarger);
    LOG(WARNING) << "Slave offset is larger than mine, Slave ip: " << node.ip()
                 << ", Slave port: " << node.port() << ", Slot: " << slot_name
                 << ", filenum: " << slave_boffset.filenum()
  ...
  std::string confile =
      NewFileName(slot->Logger()->filename(), slave_boffset.filenum());
  ...
  if (seeked_offset.filenum != slave_boffset.filenum() ||
      seeked_offset.offset != slave_boffset.offset()) {
    try_sync_response->set_reply_code(
        InnerMessage::InnerResponse::TrySync::kError);
    LOG(WARNING) << "Slave offset is not a start point of cur log, Slave ip: "
                 << node.ip() << ", Slave port: " << node.port()
                 << ", Slot: " << slot_name
                 << ", cloest start point, filenum: " << seeked_offset.filenum
...
void PikaReplServerConn::BuildConsensusMeta(
    const bool& reject, const std::vector<LogOffset>& hints,
...
void PikaReplServerConn::HandleDBSyncRequest(void* arg) {
  ...
  InnerMessage::InnerResponse::DBSync* db_sync_response =
      response.mutable_db_sync();
  ...
  if (prior_success) {
    if (!master_slot->CheckSlaveNodeExist(node.ip(), node.port())) {
      ...
      if (prior_success) {
        ...
        Status s =
            master_slot->AddSlaveNode(node.ip(), node.port(), session_id);
        if (s.ok()) {
          const std::string ip_port =
              pstd::IpPortString(node.ip(), node.port());
          g_pika_rm->ReplServerUpdateClientConnMap(ip_port, conn->fd());
          LOG(INFO) << "Slot: " << slot_name
                    << " Handle DBSync Request Success, Session: "
                    << session_id;
        } else {
          response.set_code(InnerMessage::kError);
          LOG(WARNING) << "Slot: " << slot_name
                       << " Handle DBSync Request Failed, " << s.ToString();
          prior_success = false;
        }
      } else {
      ...
      int32_t session_id;
      Status s =
          master_slot->GetSlaveNodeSession(node.ip(), node.port(), &session_id);
      if (!s.ok()) {
        response.set_code(InnerMessage::kError);
        LOG(WARNING) << "Slot: " << slot_name << ", Get Session id Failed"
                     << s.ToString();
        ...
        db_sync_response->set_session_id(session_id);
        LOG(INFO) << "Slot: " << slot_name
                  << " Handle DBSync Request Success, Session: " << session_id;
      }
    }
  }

  g_pika_server->TryDBSync(node.ip(), node.port() + kPortShiftRSync, db_name,
                           slot_id, slave_boffset.filenum());
...
void PikaReplServerConn::HandleBinlogSyncRequest(void* arg) {
  ...
  const InnerMessage::BinlogOffset& ack_range_start =
      binlog_req.ack_range_start();
  const InnerMessage::BinlogOffset& ack_range_end = binlog_req.ack_range_end();
  BinlogOffset b_range_start(ack_range_start.filenum(),
                             ack_range_start.offset());
  ...
  if (req->has_consensus_meta()) {
    ...
    if (meta.term() > master_slot->ConsensusTerm()) {
      LOG(INFO) << "Update " << db_name << ":" << slot_id << " term from "
                << master_slot->ConsensusTerm() << " to " << meta.term();
  ...
  if (!master_slot->CheckSessionId(node.ip(), node.port(), db_name, slot_id,
                                   session_id)) {
  ...
  Status s =
      master_slot->SetLastRecvTime(node.ip(), node.port(), pstd::NowMicros());
  if (!s.ok()) {
    LOG(WARNING) << "SetMasterLastRecvTime failed " << node.ip() << ":"
                 << node.port() << ", " << db_name << "_" << slot_id << " "
                 << s.ToString();
    conn->NotifyClose();
    return;
  }
  ...
  if (is_first_send) {
    ...
    Status s = master_slot->ActivateSlaveBinlogSync(node.ip(), node.port(),
                                                    range_start);
    if (!s.ok()) {
      LOG(WARNING) << "Activate Binlog Sync failed " << slave_node.ToString()
                   << " " << s.ToString();
      conn->NotifyClose();
      return;
    }
  ...
  if (range_start.b_offset == BinlogOffset() &&
      range_end.b_offset == BinlogOffset()) {
    return;
  }
  s = g_pika_rm->UpdateSyncBinlogStatus(slave_node, range_start, range_end);
  if (!s.ok()) {
    LOG(WARNING) << "Update binlog ack failed " << db_name << " " << slot_id
                 << " " << s.ToString();
    conn->NotifyClose();
    return;
  }
...
void PikaReplServerConn::HandleRemoveSlaveNodeRequest(void* arg) {
  ...
  const InnerMessage::InnerRequest::RemoveSlaveNode& remove_slave_node_req =
      req->remove_slave_node(0);
  ...
  InnerMessage::InnerResponse::RemoveSlaveNode* remove_slave_node_response =
      response.add_remove_slave_node();
  InnerMessage::Slot* slot_response =
      remove_slave_node_response->mutable_slot();
  slot_response->set_db_name(db_name);
  slot_response->set_slot_id(slot_id);
  InnerMessage::Node* node_response =
      remove_slave_node_response->mutable_node();
...
int PikaReplServerConn::DealMessage() {
  std::shared_ptr<InnerMessage::InnerRequest> req =
      std::make_shared<InnerMessage::InnerRequest>();
  bool parse_res =
  ...
  switch (req->type()) {
    case InnerMessage::kMetaSync: {
      auto task_arg = new ReplServerTaskArg(
          req,
          std::dynamic_pointer_cast<PikaReplServerConn>(shared_from_this()));
      g_pika_rm->ScheduleReplServerBGTask(
          &PikaReplServerConn::HandleMetaSyncRequest, task_arg);
      break;
    }
    case InnerMessage::kTrySync: {
      auto task_arg = new ReplServerTaskArg(
          ...
          std::dynamic_pointer_cast<PikaReplServerConn>(shared_from_this()));
      g_pika_rm->ScheduleReplServerBGTask(
          &PikaReplServerConn::HandleTrySyncRequest, task_arg);
      break;
    }
    case InnerMessage::kDBSync: {
      auto task_arg = new ReplServerTaskArg(
          ...
          std::dynamic_pointer_cast<PikaReplServerConn>(shared_from_this()));
      g_pika_rm->ScheduleReplServerBGTask(
          &PikaReplServerConn::HandleDBSyncRequest, task_arg);
      break;
    }
    case InnerMessage::kBinlogSync: {
      auto task_arg = new ReplServerTaskArg(
          ...
          std::dynamic_pointer_cast<PikaReplServerConn>(shared_from_this()));
      g_pika_rm->ScheduleReplServerBGTask(
          &PikaReplServerConn::HandleBinlogSyncRequest, task_arg);
      break;
    }
    case InnerMessage::kRemoveSlaveNode: {
      auto task_arg = new ReplServerTaskArg(
          ...
          std::dynamic_pointer_cast<PikaReplServerConn>(shared_from_this()));
      g_pika_rm->ScheduleReplServerBGTask(
          &PikaReplServerConn::HandleRemoveSlaveNodeRequest, task_arg);
      break;
    }
    default:
      break;
