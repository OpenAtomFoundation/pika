...
void SubscribeCmd::Do(std::shared_ptr<Slot> slot) {
  ...
  g_pika_server->Subscribe(conn, channels, name_ == kCmdNamePSubscribe,
                           &result);
...
void UnSubscribeCmd::Do(std::shared_ptr<Slot> slot) {
  ...
  std::vector<std::pair<std::string, int>> result;
  int subscribed = g_pika_server->UnSubscribe(
      conn, channels, name_ == kCmdNamePUnSubscribe, &result);
...
void PubSubCmd::DoInitial() {
  ...
  if (strcasecmp(subcommand_.data(), "channels") != 0 &&
      strcasecmp(subcommand_.data(), "numsub") != 0 &&
