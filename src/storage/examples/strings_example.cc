...
int main() {
  ...
  printf("GetSet return: %s, old_value: %s", s.ToString().c_str(),
         value.c_str());
  ...
  // and
  s = db.BitOp(storage::BitOpType::kBitOpAnd, "BITOP_DESTKEY", src_keys,
               &bitop_ret);
  printf("BitOp return: %s, ret: %d\n", s.ToString().c_str(), bitop_ret);
  // or
  s = db.BitOp(storage::BitOpType::kBitOpOr, "BITOP_DESTKEY", src_keys,
               &bitop_ret);
  printf("BitOp return: %s, ret: %d\n", s.ToString().c_str(), bitop_ret);
  // xor
  s = db.BitOp(storage::BitOpType::kBitOpXor, "BITOP_DESTKEY", src_keys,
               &bitop_ret);
  printf("BitOp return: %s, ret: %d\n", s.ToString().c_str(), bitop_ret);
  // not
  std::vector<std::string> not_keys{"BITOP_KEY1"};
  s = db.BitOp(storage::BitOpType::kBitOpNot, "BITOP_DESTKEY", not_keys,
               &bitop_ret);
  printf("BitOp return: %s, ret: %d\n", s.ToString().c_str(), bitop_ret);
