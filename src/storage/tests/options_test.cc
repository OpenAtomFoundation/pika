...
TEST_F(StorageOptionsTest, ResetOptionsTest) {
  std::unordered_map<std::string, std::string> cf_options_map{
      {"write_buffer_size", "4096"}, {"max_write_buffer_number", "10"}};
  ...
  std::unordered_map<std::string, std::string> invalid_cf_options_map{
      {"write_buffer_size", "abc"}, {"max_write_buffer_number", "0x33"}};
  s = storage_options.ResetOptions(OptionType::kColumnFamily,
                                   invalid_cf_options_map);
  ASSERT_FALSE(s.ok());
  ASSERT_EQ(storage_options.options.write_buffer_size, 4096);
  ASSERT_EQ(storage_options.options.max_write_buffer_number, 10);

  std::unordered_map<std::string, std::string> db_options_map{
      {"max_open_files", "16"}, {"max_background_compactions", "32"}};
  ...
  std::unordered_map<std::string, std::string> invalid_db_options_map{
      {"max_open_files", "a"}, {"max_background_compactions", "bac"}};
