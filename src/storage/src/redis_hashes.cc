...
Status Redis::HDel(const Slice& key, const std::vector<std::string>& fields, int32_t* ret) {
  ...
  if (s.ok() && !ExpectedMetaValue(Type::kHash, meta_value)) {
    if (ExpectedStale(meta_value)) {
      ...
      return Status::InvalidArgument("WRONGTYPE, key: " + key.ToString() + ", expect type: " + TypeStrings[static_cast<int>(Type::kHash)] + "get type: " + TypeStrings[static_cast<int>(GetMetaValueType(meta_value))]);
    }
  }
  if (s.ok()) {
...
void Redis::ScanHashes() {
  ...
  for (meta_iter->SeekToFirst(); meta_iter->Valid(); meta_iter->Next()) {
    if (!ExpectedMetaValue(Type::kHash, meta_iter->value().ToString())) {
      continue;
    }
