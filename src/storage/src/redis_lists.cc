...
RedisLists::RedisLists(Storage* const s, const DataType& type)
    : Redis(s, type) {}

Status RedisLists::Open(const StorageOptions& storage_options,
...
Status RedisLists::PKPatternMatchDel(const std::string& pattern, int32_t* ret) {
  ...
  while (iter->Valid()) {
    ...
    if (!parsed_lists_meta_value.IsStale() &&
        (parsed_lists_meta_value.count() != 0U) &&
...
Status RedisLists::LIndex(const Slice& key, int64_t index,
                          std::string* element) {
  ...
  if (s.ok()) {
    ...
    if (parsed_lists_meta_value.IsStale()) {
      ...
      uint64_t target_index =
          index >= 0 ? parsed_lists_meta_value.left_index() + index + 1
      ...
      if (parsed_lists_meta_value.left_index() < target_index &&
        ...
        s = db_->Get(read_options, handles_[1], lists_data_key.Encode(),
                     &tmp_element);
        if (s.ok()) {
          *element = tmp_element;
        }
      ...
      if (!find_pivot) {
        *ret = -1;
        ...
        if (pivot_index <= mid_index) {
          target_index =
              (before_or_after == Before) ? pivot_index - 1 : pivot_index;
          ...
          target_index =
              (before_or_after == Before) ? pivot_index : pivot_index + 1;
...
Status RedisLists::LPop(const Slice& key, std::string* element) {
  ...
  if (s.ok()) {
    ...
    if (parsed_lists_meta_value.IsStale()) {
      ...
      s = db_->Get(default_read_options_, handles_[1], lists_data_key.Encode(),
                   element);
      if (s.ok()) {
...
Status RedisLists::LPushx(const Slice& key,
                          const std::vector<std::string>& values,
                          uint64_t* len) {
  *len = 0;
  rocksdb::WriteBatch batch;
...
Status RedisLists::LRange(const Slice& key, int64_t start, int64_t stop,
  ...
  if (s.ok()) {
    ...
    if (parsed_lists_meta_value.IsStale()) {
      ...
      uint64_t sublist_left_index = start >= 0 ? origin_left_index + start
                                               : origin_right_index + start + 1;
      uint64_t sublist_right_index =
          stop >= 0 ? origin_left_index + stop : origin_right_index + stop + 1;
    ...
    if (parsed_lists_meta_value.IsStale()) {
      ...
      if (target_index.empty()) {
        *ret = 0;
        ...
        uint64_t sublist_left_index =
            (count >= 0) ? target_index[0]
                         : target_index[target_index.size() - 1];
        uint64_t sublist_right_index =
            (count >= 0) ? target_index[target_index.size() - 1]
                         : target_index[0];
...
Status RedisLists::LSet(const Slice& key, int64_t index, const Slice& value) {
  ...
  if (s.ok()) {
    ...
    if (parsed_lists_meta_value.IsStale()) {
      ...
      ListsDataKey lists_data_key(key, version, target_index);
      s = db_->Put(default_write_options_, handles_[1], lists_data_key.Encode(),
                   value);
      statistic++;
      UpdateSpecificKeyStatistics(key.ToString(), statistic);
      return s;
  ...
  if (s.ok()) {
    ...
    if (parsed_lists_meta_value.IsStale()) {
      ...
      if (sublist_left_index > sublist_right_index ||
        ...
        batch.Put(handles_[0], key, meta_value);
        for (uint64_t idx = origin_left_index; idx < sublist_left_index;
             ++idx) {
          statistic++;
          ListsDataKey lists_data_key(key, version, idx);
          batch.Delete(handles_[1], lists_data_key.Encode());
        }
        for (uint64_t idx = origin_right_index; idx > sublist_right_index;
             --idx) {
          statistic++;
          ListsDataKey lists_data_key(key, version, idx);
          batch.Delete(handles_[1], lists_data_key.Encode());
...
Status RedisLists::RPoplpush(const Slice& source, const Slice& destination,
                             std::string* element) {
  element->clear();
  uint32_t statistic = 0;
  Status s;
  ...
  s = db_->Get(default_read_options_, handles_[0], destination,
               &destination_meta_value);
  if (s.ok()) {
...
Status RedisLists::RPushx(const Slice& key,
                          const std::vector<std::string>& values,
                          uint64_t* len) {
  *len = 0;
  rocksdb::WriteBatch batch;
...
bool RedisLists::Scan(const std::string& start_key, const std::string& pattern,
...
bool RedisLists::PKExpireScan(const std::string& start_key,
  ...
  while (it->Valid() && (*leftover_visits) > 0) {
    ...
    if (parsed_lists_meta_value.IsStale() ||
      ...
      if (min_timestamp < parsed_lists_meta_value.timestamp() &&
          parsed_lists_meta_value.timestamp() < max_timestamp) {
...
void RedisLists::ScanDatabase() {
  ...
  for (meta_iter->SeekToFirst(); meta_iter->Valid(); meta_iter->Next()) {
    ...
    LOG(INFO) << fmt::format(
        "[key : {:<30}] [count : {:<10}] [left index : {:<10}] [right index : "
        "{:<10}] [timestamp : {:<10}] [version : "
        "{}] [survival_time : {}]",
        meta_iter->key().ToString(), parsed_lists_meta_value.count(),
        parsed_lists_meta_value.left_index(),
        parsed_lists_meta_value.right_index(),
        parsed_lists_meta_value.timestamp(), parsed_lists_meta_value.version(),
        survival_time);
  }
  delete meta_iter;
  ...
  for (data_iter->SeekToFirst(); data_iter->Valid(); data_iter->Next()) {
    ...
    LOG(INFO) << fmt::format(
        "[key : {:<30}] [index : {:<10}] [data : {:<20}] [version : {}]",
