...
Status Redis::LIndex(const Slice& key, int64_t index, std::string* element) {
  ...
  if (s.ok() && !ExpectedMetaValue(Type::kList, meta_value)) {
    if (ExpectedStale(meta_value)) {
      ...
      return Status::InvalidArgument("WRONGTYPE, key: " + key.ToString() + " What Type: List");
    }
  }
  if (s.ok()) {
...
Status Redis::RPoplpush(const Slice& source, const Slice& destination, std::string* element) {
  ...
  if (source.compare(destination) == 0) {
    ...
    if (s.ok() && !ExpectedMetaValue(Type::kList, meta_value)) {
      if (ExpectedStale(meta_value)) {
        ...
        return Status::InvalidArgument("WRONGTYPE, key: " + destination.ToString() + " What Type: List");
      }
    }
    if (s.ok()) {
      ParsedListsMetaValue parsed_lists_meta_value(&meta_value);
  ...
  if (s.ok() && !ExpectedMetaValue(Type::kList, source_meta_value)) {
    if (ExpectedStale(source_meta_value)) {
      ...
      return Status::InvalidArgument("WRONGTYPE, key: " + source.ToString() + " What Type: List");
    }
  }
  if (s.ok()) {
