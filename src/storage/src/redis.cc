...
Redis::Redis(Storage* const s, const DataType& type)
  ...
  statistics_store_ = std::shared_ptr<LRUCache<std::string, size_t>> (new LRUCache<std::string, size_t>());
  scan_cursors_store_ = std::shared_ptr<LRUCache<std::string, std::string>>(new LRUCache<std::string, std::string>());
