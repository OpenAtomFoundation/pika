...
Storage::Storage()
  ...
  if (!s.ok()) {
    LOG(ERROR) << "[FATAL] start bg thread failed, " << s.ToString().c_str();
    exit(-1);
  }
}
...
Status Storage::Open(const StorageOptions& storage_options, const std::string& db_path) {
  ...
  if (!s.ok()) {
    LOG(ERROR) << "[FATAL] open kv db failed, " << s.ToString().c_str();
    exit(-1);
  }
  ...
  if (!s.ok()) {
    LOG(ERROR) << "[FATAL] open hashes db failed, " << s.ToString().c_str();
    exit(-1);
  }
  ...
  if (!s.ok()) {
    LOG(ERROR) << "[FATAL] open set db failed, " << s.ToString().c_str();
    exit(-1);
  }
  ...
  if (!s.ok()) {
    LOG(ERROR) << "[FATAL] open list db failed, " << s.ToString().c_str();
    exit(-1);
  }
  ...
  if (!s.ok()) {
    LOG(ERROR) << "[FATAL] open zset db failed, " << s.ToString().c_str();
    exit(-1);
  }
